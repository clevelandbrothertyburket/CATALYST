...
// Codes are either Active or Paused; Remove hard-deletes them (see deleteCode).
...
export const USABLE_STATUSES = ['active'];
...
export async function approveRequest({ requestId, approver, decisionNote }) {
  ...
  if (req.kind === 'create_code') {
    ...
    return { ok: true, codeId: id, code: p.code, campName: p.campName, businessUnit: buRow[0].name, initiative: initRow[0].name, requestedBy: req.requested_by };
  }
...
// Permanently delete a code from the system (Remove). Hard delete + audit.
export async function deleteCode({ codeId, actor }) {
  const rows = await sql`SELECT code, status FROM codes WHERE id=${codeId}`;
  if (!rows.length) return { error: 'Code not found', status: 404 };
  await sql`DELETE FROM codes WHERE id=${codeId}`;
  await audit({ actor: actor.name, action: 'code.removed', entity: 'code', entityId: codeId,
    before: { code: rows[0].code, status: rows[0].status } });
  return { ok: true, removed: rows[0].code };
}
