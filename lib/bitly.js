...
// Detail analytics for an arbitrary bitlink id (used by the link detail modal).
export async function linkDetail(bitlink) {
  return bitlyAnalytics(bitlink);
}

// Attempt to list dedicated QR Codes for the group (available on some plans).
// Best-effort: returns [] if the endpoint isn't available on this plan.
export async function listQrCodes() {
  try {
    const guid = await groupGuid();
    const d = await bitly('/groups/' + guid + '/qr-codes');
    const arr = d.qr_codes || d.qrcodes || [];
    return arr.map((q) => ({
      id: q.qrcode_id || q.id,
      title: q.title || '',
      link: (q.bitlink && (q.bitlink.link || q.bitlink.id)) || q.link || '',
      long_url: (q.bitlink && q.bitlink.long_url) || q.long_url || '',
      created: q.created || q.created_at || null,
    }));
  } catch { return []; }
}
